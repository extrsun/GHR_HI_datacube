pro accum_mom_5461,velbegin=velbegin,velend=velend,ymin=ymin,ymax=ymax, $
  subdir=subdir,binsize=binsize
;+
; Name
;      accum_mom_5461
; Purpose
;      Calculate the accumulative momenta at different positions 
...
;-
  ...
  if subdir eq 'wide' then flucdmom=[0.5,0.15] else flucdmom=[0.5,0.2]
  flucdmom*=1e6
  ...
  divfac_mom=double(1e23)*1e20 ;Dividing factor of kinetic energy
  divfac_momd=1.0e6 ;Dividing factor of density of kinetic energy
  ...
  charsize=1.5 & thick=5 & ancthick=3 & lchsize=1.2 & momchsize=1.8
  ...
  grmomv1=dblarr(xnum,ynum) & grmomv2=dblarr(xnum,ynum)
  ...
  for i=0,nsublist-1 do begin    
    filepath=genpath+'accum_mom/'+sublist[i]+'/'
    subpath=filepath+subdir+'/' & figpath=subpath+'figures/'
    ;Open the ascii files used to record the kinetic energy
    openu,lun1,subpath+'mom1105_'+sublist[i]+'_'+subdir+'.info',/get_lun,/append
    openu,lun2,subpath+'mom1098_'+sublist[i]+'_'+subdir+'.info',/get_lun,/append
    ...
    for k=0,xnum-1 do begin
      ...
      momval=total(fdata*abs(velvalarr-cenvel),3)
      grmomv1[k,*]=total(momval,1)
      ...
      grmomv2[k,*]=total(momval,1)
    endfor
    accummom1=total(grmomv1,2)/pixarea
    accummom2=total(grmomv2,2)/pixarea
    grmomv1/=binsize & grmomv2/=binsize
    
    ;Plot the kinematic energy profile along y-axis at variant positions
    device,filename=figpath+'momprof_'+sublist[i]+'_'+subdir+'_'+ $
    ...
    plymin1=min([grmomv1[illran,*],grmomv2[illran,*]],max=plymax1)
    plydel1=plymax1-plymin1
    yrange1=[plymin1-plydel1*1e-4,plymax1+plydel1*0.2]/divfac_mom
    ...
    for q=0,1 do begin
      for p=0,2 do begin
        ...
        if p eq 0 then ytitle='!6E!Ik!N (10!U'+ $
            strtrim(string(round(alog10(divfac_mom))),2)+ $
        ...
        if p eq 0 then begin
          if q eq 0 then $
            plot,xrange,yrange1,xrange=xrange,/nodata,yrange=yrange1, $
              ...
              charsize=momchsize,xtickname=xtickname,position=position $
          else $
            plot,xrange,yrange1,xrange=xrange,/nodata,yrange=yrange1, $
              xstyle=1,ystyle=9,xtitle=xtitle,ytitle=ytitle, $
              charsize=momchsize,position=position
        endif else begin
          if q eq 0 then $
            plot,xrange,yrange1,xrange=xrange,/nodata,yrange=yrange1, $
              xstyle=1,ystyle=9,xtitle=xtitle,ytitle=ytitle, $
              position=position,charsize=momchsize,xtickname=xtickname, $
              ytickname=ytickname $
          else $
            plot,xrange,yrange1,xrange=xrange,/nodata,yrange=yrange1, $
              xstyle=1,ystyle=9,xtitle=xtitle,ytitle=ytitle, $
              charsize=momchsize,ytickname=ytickname,position=position
        endelse
        for m=0,illnum-1 do begin
          pldat=grmomv1[illind[m],*]/divfac_mom & selind=where(pldat gt 0.0)
          oplot,yind[selind],pldat[selind],color=colind[m], $
            thick=thkind[m],linestyle=2
          pldat=grmomv2[illind[m],*]/divfac_mom & selind=where(pldat gt 0.0)
          oplot,yind[selind],pldat[selind],color=colind[m], $
            thick=thkind[m],linestyle=1
        endfor
        if p eq 2 then $
          axis,/yaxis,yrange=yrange2,/save,ystyle=1,charsize=momchsize, $
    ...
    device,filename=figpath+'acmompf_'+sublist[i]+'_'+subdir+'_'+ $
      string(binsize,format='(i02)')+'_r51.eps'
    xind=(xindl+xindr)/2.;-xcenter
    xrange=[xind[0],xind[xnum-1]]
    plymin1=min([accummom1,accummom2],max=plymax1)
    plymin1/=divfac_mom & plymax1/=divfac_mom & plydel1=plymax1-plymin1
    ...
    oplot,xind,accummom1/divfac_mom,thick=thick,color=1
    oplot,xind,accummom2/divfac_mom,thick=thick,color=2
    ...
    device,filename=figpath+'acdmompf_'+sublist[i]+'_'+subdir+'_'+ $
      string(binsize,format='(i02)')+'_r51.eps'
    xind=(xindl+xindr)/2.;-xcenter
    xrange=[xind[0],xind[xnum-1]]
    pldat1=accummom1/(selmass*!msolar)/divfac_momd
    pldat2=accummom2/(selmass*!msolar)/divfac_momd
    ...
    plot,xrange,yrange1,/nodata,xrange=xrange1,yrange=yrange,xstyle=1, $
      ...
      ystyle=9,ytitle='Kinetic Energy Density (10!U'+ $
        strtrim(string(round(alog10(divfac_momd))),2)+ $
    ...
    ;along the X-axis
    ; pldata3=(pldat2*divfac_momd-flucdmom[i])*selmass*!msolar/divfac_mom
    pldata1=(accummom2-selmass*!msolar*flucdmom[i])/divfac_mom
    pldata2=selmass/divfac_mass
    device,filename=figpath+'acnetmompf_'+sublist[i]+'_'+subdir+'_'+ $
    ...
    for m=0,1 do begin
      ...
      netval=total(pldata1[selind])*divfac_mom*pixarea
      rawval=total(accummom2[selind])*pixarea
      subval=total(selmass[selind]*!msolar*flucdmom[i])*pixarea
